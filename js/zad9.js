function buildSDNF() {
    ...
    if (length === 0 || (length & (length - 1)) !== 0) {
        outputElement.textContent = 'Ошибка: Длина вектора должна быть степенью двойки (2, 4, 8, 16, ...).';
        return;
    }
    ...
    for (let i = 0; i < length; i++) {
        if (vectorInput[i] === '0') {
            ...
            for (let j = 0; j < n; j++) {
                const bit = binary[j];
                const styledVar = bit === '1' ? `<span style="text-decoration: overline">${variables[j]}</span>` : variables[j];
    ...
    // Измените вывод на СКНФ
    outputElement.innerHTML = terms.length > 0 
        ? `СКНФ: ${terms.join(' ∧ ')}` 
        : 'СКНФ: 1'; // Если нет нулей
}