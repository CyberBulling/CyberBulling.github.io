function VectorFunction(zeroResidual, oneResidual, argumentIndex, number_of_variables) {
    let resultVector = [];
    if (argumentIndex === 1) {
...
zeroResidualInput.addEventListener("change", (event) => {
    ...
    if (!isBinary(input)) {
        ...
        const filtered = input.split('').filter(char => char === '0' || char === '1').join('');
        // Обновляем значение в поле ввода
        event.target.value = filtered;
        // Показываем предупреждение только если были удалены символы
        if (filtered !== input) {
            alert('Неверный ввод. Разрешены только 0 и 1');
        }
    }
});
...
function outputText() {
    ...
    if (zeroResidual.length !== oneResidual.length) {
        alert("Длины векторов должны совпадать");
    }
    ...
    // Получаем результат
    const vector = VectorFunction(zeroResidual, oneResidual, argumentIndex, number_of_variables);
