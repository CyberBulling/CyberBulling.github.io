...
function buildDNF () {
  ...
  if (numVars % 1 !== 0) {
    notyf.error('Некорректная длина вектора')
    return
  }
  ...
  // Удаление дубликатов
  const uniqueImplicants = currentTerms.filter(
    (term, index, self) =>
      self.findIndex(t => t.every((val, idx) => val === term[idx])) === index
  )

  // Жадный алгоритм для покрытия
  const remainingTerms = ones.map(idx => idx.toString(2).padStart(numVars, '0'))
      ...
      .map(imp => {
        const vars = imp
          .map((val, idx) => {
            ...
            if (val === 0)
              return `<span style="text-decoration: overline">${variable}</span>`
